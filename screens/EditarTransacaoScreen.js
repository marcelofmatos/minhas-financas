...
//
// Tela de edição de uma transação existente. Duplicação consciente do JSX da
// NovaTransacaoScreen (decisão de design): mesmo formulário, mas:
//  - useState inicializado a partir de route.params.transacao
//  - chama editarTransacao (UPDATE) em vez de adicionarTransacao
//  - data e id originais são preservados
//  - ao salvar, navigation.replace para a tela de detalhe com os dados novos
...
export function EditarTransacaoScreen({ route, navigation }) {
  ...
  const [localizacao, setLocalizacao] = useState(
    transacao.latitude != null && transacao.longitude != null
      ? { latitude: transacao.latitude, longitude: transacao.longitude }
      : null
  );
  const [modalVisivel, setModalVisivel] = useState(false);
  const [comprovante, setComprovante] = useState(transacao.comprovante ?? null);

  const { editarTransacao } = useTransacoes();
  ...
  const salvar = async () => {
    ...
    await editarTransacao(atualizada);
    navigation.replace('DetalheTransacao', { transacao: atualizada });
  };

  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      <Text style={styles.tituloPagina}>Editar Transação</Text>
